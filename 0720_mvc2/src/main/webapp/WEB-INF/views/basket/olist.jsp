...
<html>
				...
				<table class="table table-hover">
					<tr>
						<td>
							...
							<button id="delivery_btn" type="button" class="btn btn-primary btn-lg"
						...
						<td id="td_delivery">
							<c:choose>
								<c:when test="${deliverylist != null && deliverylist[0] != null}">
									<h5> 배송지 : ${deliverylist[0].addr_name} </h5>
									<h6> 받는 분 : ${deliverylist[0].recipient_name} ( 연락처 : ${deliverylist[0].tel} ) </h6>
									<h6> ( ${deliverylist[0].post_code} ) ${deliverylist[0].addr1} ${deliverylist[0].addr2} </h6>
								</c:when>
								<c:otherwise>
									<h5> 등록된 배송지가 없습니다. </h5>
								</c:otherwise>
				...
				<table class="table table-hover">
					<tr>
						<td>
							...
							<button id="credit_card_btn" type="button" class="btn btn-primary btn-lg"
						...
						<td id="td_credit_card">
							<c:choose>
								<c:when test="${cardlist != null && cardlist[0] != null}">
									<h5> ${cardlist[0].card_name} </h5>
									<h6> 카드 번호 : ${cardlist[0].card_number} </h6>
									<h6> 유효 기간 (MM/YY) : ${cardlist[0].expiry_date_month} / ${cardlist[0].expiry_date_year} </h6>
								</c:when>
								<c:otherwise>
									<h5> 등록된 카드가 없습니다. </h5>
								</c:otherwise>
		...
		<div class="modal-dialog">
			<div class="modal-content">
							...
							<c:forEach var="dto" items="${deliverylist}" varStatus="status">
								<tr>
									<td id="td_delivery${status.index}">
										...
										<h6> 받는 분 : ${dto.recipient_name} ( 연락처 : ${dto.tel} ) </h6>
										<h6> ( ${dto.post_code} ) ${dto.addr1} ${dto.addr2} </h6>
									</td>
									<td class="text-right">
										<button class="addr_delete_btn btn btn-danger btn-sm" value="${dto.addr_no}"> 삭제 </button>
										<br>
										<br>
				...
				<div class="modal-footer">
					...
					<button type="button" class="btn btn-warning btn-lg" data-dismiss="modal"> 취 소 </button>
				</div>
						...
						<tbody>
							<c:forEach var="dto" items="${cardlist}" varStatus="status">
								<tr>
									...
									<td class="text-right">
										<button class="credit_card_delete_btn btn btn-danger btn-sm" value="${dto.card_no}"> 삭제 </button>
										<br>
										<br>
	...
	$(document).ready(function() {
		$(".addr_delete_btn").click(function() {
			...
			$.get(
					"${pageContext.request.contextPath}/delivery/delcredit_card"
					, {
						addr_no : $(this).val()
					}
					, function(data, status) {
						if(data >= 1){
							...
							location.href="${pageContext.request.contextPath}/basket/orderList";
						} else {
