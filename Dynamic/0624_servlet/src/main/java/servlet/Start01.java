...
@WebServlet("/end")//program id : 유저는 프로그램 아이디를 서버로 전송해서 servlet 프로그램을 실행시킨다.
//하나의 servlet 프로그램에는 하나의 program id가 필요하다.
//@ : annotation : 어노테이션
public class Start01 extends HttpServlet {
	//Servlet 프로그램은 유저에게 전송할 웹페이지를 만든다.
	//Servlet 프로그램은 반드시 extends HttpServlet을 해야 한다.
	/*
	 * HttpServlet : http 통신을 하는 servlet 객체
	 *	- Tomcat으로부터 유저 정보와 유저가 보낸 기타 정보를 받는다.
	 *	- Tomcat에게 유저에게 전송할 html 문서를 준다.
